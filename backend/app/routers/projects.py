...
# 2. GEREKLİ ŞEMALAR (YENİ ŞEMALARI IMPORT EDECEĞİZ)
from app.schemas import project_schemas
from app.schemas.project_member_schemas import ProjectMemberInvite, ProjectMemberDisplay, ProjectMemberUpdate # ProjectMemberUpdate eklendi
...
def get_membership_by_id(
    project_id: int, 
    member_id: int, # Bu, user_id değil, 'project_members' tablosunun kendi ID'si
    db: Session
) -> ProjectMember:
...
def get_project_by_id(membership: ProjectMember = Depends(get_project_membership)):
    # ... (Mevcut get_project_by_id kodun) ...
...
def add_member_to_project(invite_data: ProjectMemberInvite, admin_membership: ProjectMember = Depends(get_project_admin), db: Session = Depends(get_db)):
    # ... (Mevcut add_member_to_project kodun) ...
...
def update_member_role(
    ...
    Sadece o projenin 'admin'leri bu işlemi yapabilir.
    """
    # Güncellenecek üyeliği bul
    membership_to_update = get_membership_by_id(project_id, member_id, db)
    
    # Güvenlik: Admin, kendisi dışındaki son adminin rolünü 'member' yapamaz
    # (Bu, projenin "sahipsiz" kalmasını önler)
    # (Bu mantığı şimdilik basitleştiriyoruz, ileride eklenebilir)
...
def remove_member_from_project(
    ...
    Sadece o projenin 'admin'leri bu işlemi yapabilir.
    """
    # Silinecek üyeliği bul
    membership_to_delete = get_membership_by_id(project_id, member_id, db)
    
    # Güvenlik: Admin kendini projeden atamaz (kendini atmak yerine projeyi silmeli)
